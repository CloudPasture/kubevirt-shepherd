...
| **AuthProviders Schema** | `ent/schema/auth_providers.go` | ⬜ | [ADR-0015](../../adr/ADR-0015-governance-model-v2.md) §22.6, [master-flow Stage 2.B](../interaction-flows/master-flow.md) ³ |
...
> ³ **V1 Scope**: IdP authentication (OIDC + LDAP) is implemented in V1; SAML is reserved for future.
...
> **Auth Adapter Reference**: [examples/provider/auth_adapter.go](../examples/provider/auth_adapter.go) (standardized provider output per ADR-0026)
