...
│                                                                                              │
│  👆 验证逻辑:                                                                                │
│     1. request ≠ limit 且环境为 prod → ⚠️ 黄色警告 (仅提示，不阻止审批)                      │
│     2. 超卖 + 专用 CPU 同时启用 → ❌ 错误 (阻止审批) ²                                         │
│        KubeVirt 要求 requests.cpu == limits.cpu 才能启用 dedicatedCpuPlacement (Guaranteed QoS)│
│                                                                                              │
│  ² **技术约束**: KubeVirt 要求 `dedicatedCpuPlacement` 必须具有 Guaranteed QoS 类，即 CPU      │
│    request 必须等于 limit。这是 K8s/KubeVirt 的硬性约束，无法绕过。详见 KubeVirt 计算文档。       │
│                                                                                              │
└─────────────────────────────────────────────────────────────────────────────────────────────┘
                                           │
                                           ▼
